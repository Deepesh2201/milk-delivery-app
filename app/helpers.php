...
function Products(){
    $contents = Product::get(['id', 'name', 'group']);	  
  return $contents;
}
function Onloadings(){
...
function OnloadedProducts(){
    ...
    $productlist = DB::table('onloadings')
    ->join('product_onloading_relations', 'onloadings.id', '=', 'product_onloading_relations.onloading_id')// joining the tables
    ->join('products', 'product_onloading_relations.product_id', '=', 'products.id')->
    where('salesman_id', Auth::id())->whereDate('onloadings.created_at', $datenow)->where('is_approved', '2')// joining the tables
    ->get();
