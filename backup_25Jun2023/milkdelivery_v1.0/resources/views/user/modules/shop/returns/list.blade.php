    ...
    <div class="row align-items-center">
        ...
        <div class="col-sm-6">
            <div class="float-right d-md-block">
                <a href="{{ route('return-product.create')}}">
                    <button class="btn btn-primary arrow-none waves-effect waves-light" type="button">
                        <i class="fas fa-plus"></i> {{ __('l.add') }} {{ __('l.return')}}
                    </button>
                </a>
            </div>
        </div>
...
<div class="row">
    <div class="col-md-12">
        <div class="card">
            <div class="card-body">
                <h4 class="mt-0 header-title m-b-30">{{ __('l.return') }} {{ __('l.list') }}</h4>
                @include('admin.partials.messages')
                <div class="table-responsive table-full-width">
                    <div style="overflow: auto;">
                        <form action="{{ route('return.product.search') }}" method="GET">                        
                        <!-- <div class="row"> -->
                            <div class="col-md-12">
                        ...
                        <table class="table table-striped">
                            <thead>
                                <tr>
                                    <th>{{ __('l.#') }}</th>
                                    <th>{{ __('l.customer').' '.__('l.name') }}</th>
                                    <th>{{ __('l.customer').' '.__('l.contact').' '.__('l.number') }}</th>
                                    <th>{{ __('l.user').' '.__('l.name') }}</th>
                                    <th>{{ __('l.user').' '.__('l.contact').' '.__('l.number') }}</th>
                                    <th>{{ __('l.product').' '.__('l.sap_id') }}</th>
                                    <th>{{ __('l.product').' '. __('l.name') }}</th>
                                    <th>{{ __('l.status') }}</th>
                                    <th>{{ __('l.product').' '. __('l.qty') }}</th>
                                    <th>{{ __('l.unit_price') }}</th>
                                    <th>{{ __('l.total_price') }}</th>
                                    <th>{{ __('l.actions') }}</th>
                                   
                                </tr>
                            </thead>
                            <tbody id="tbody">
                                @include('admin.modules.shop.returns.tbody')
                            </tbody>
                        </table>
                    </div>
                </div>
