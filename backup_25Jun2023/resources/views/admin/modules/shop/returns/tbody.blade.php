...
@if($productArr->count() >= 1 )
    ...
    <td>
        <div class="btn-group" role="group" aria-label="...">
            <a href="{{ route('return-product.edit',$item->id)}}" class="btn btn-default"><i class="far fa-edit"></i></a>
            <a href="{{ route('return.product.delete',$item->id)}}" class="btn btn-default" ><i class="far fa-trash-alt"></i></a>           
        </div>
    </td>
</tr>
@foreach($productArr as $p)
    <tr>
        ...
        <td>{{$item->salesman->name ?? ''}}</td>
        <td>{{$item->salesman->contact_number ?? ''}}</td>
        <td>{{$p->returnProducts->sap_id ?? ''}}</td>
        <td>{{$p->returnProducts->name ?? ''}}</td>
        ...
        <!-- <td> -->
            <!-- <div class="btn-group" role="group" aria-label="...">
                <a href="{{ route('return-product.edit',$p->id)}}" class="btn btn-default"><i class="far fa-edit"></i></a>
                <a id="{{ $p->id }}" onclick="deleteItem( this.id )" class="btn btn-default"><i class="far fa-trash-alt"></i></a>
                <a href="{{ route('return.product.delete',$p->id)}}" class="btn btn-default" ><i class="far fa-trash-alt"></i></a>
...
@endforeach
@else
        <td>{{$item->oneLine->returnProducts->sap_id ?? ''}}</td>
        <td>{{$item->oneLine->returnProducts->name ?? ''}}</td>
        <td>{{$item->sta->returnProducts->name ?? ''}}</td>
        <td>{{ isset($item->status) && $item->status >= 1 ? 'Returned' : 'Sold'}}</td>
        <td>{{$item->qty ?? ''}}</td>
